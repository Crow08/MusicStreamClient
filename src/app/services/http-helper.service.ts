...
export class HttpHelperService {
    ...
    return new Promise<T[]>((resolve, reject) => {
      this.http.get(`http://${environment.dbServer}${path}`, options)
        .subscribe(valueArray => {
          const resultArray: T[] = [];
          (valueArray as any[]).forEach((rawArtist) => resultArray.push(plainToClass(clazz, rawArtist)));
          resolve(resultArray);
        }, reject);
    });
  }
  ...
  public post<T>(path: string, body: any): Promise<T> {
    const options = {headers: this.authenticationService.getAuthHeaderForCurrentUser()};
    return new Promise<T>((resolve, reject) => {
      this.http.post(`http://${environment.dbServer}${path}`, body, options).subscribe(resolve, reject);
    });
  }
